...
private IMAuthSNSInfo facebookSnInfo;
	
private bool googleConnected = false;
private IMAuthSNSInfo googleSnInfo;
	
private bool gameCenterConnected = false;
private IMAuthSNSInfo gameCenterSnInfo;
	
IMSDKApi.Auth.GetConnectInfo(delegate(IMAuthResult authResult) {
	if(authResult.IMSDKRetCode == RET_SUCCESS) {
...
* 并且迁移码只可以用于Android和iOS之间，比如在Android A设备获取迁移码之后，在Android B设备使用该迁移码，后台会返回提示：`the same platform!`
